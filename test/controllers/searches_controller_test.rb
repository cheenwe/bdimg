...
class SearchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create search" do
    assert_difference('Search.count') do
      post searches_url, params: { search: { name: @search.name, number: @search.number, total: @search.total } }
    end
  ...
  test "should update search" do
    patch search_url(@search), params: { search: { name: @search.name, number: @search.number, total: @search.total } }
